  ...
  const handleDeleteRegulations = async (id: number) => {
    ...
    try {
      const result = await Swal.fire({
        title: "Apakah Anda Yakin Menghapus Regulasi?",
        text: "Regulasi yang telah dihapus tidak dapat dipulihkan!",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteRegulations(id);

        if (response.status === 200) {
          await Swal.fire({
            icon: "success",
            title: `Regulasi berhasil dihapus!`,
            timer: 2000,
            position: "center",
          });
