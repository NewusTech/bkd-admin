...
import SuperUploadBKDMasterDataTablePages from "@/components/tables/master_datas/upload_bkd_table/page";
import MobileUploadBKDMasterDataCard from "@/components/mobile_all_cards/mobileUploadBkdMasterDataCard";

export default function UploadBKDStructureScreen() {
  ...
  const fetchBKDUpload = async () => {
    try {
      const response = await getBKDStructure();
  ...
  const handleCreateRegulations = async (
    ...
    try {
      const response = await createBKDStructure(formData);
      ...
      if (response.status === 201) {
        ...
        Swal.fire({
          icon: "success",
          title: "Berhasil mengunggah Struktur BKD!",
          timer: 2000,
        ...
        router.push("/super-admin/master-data/bkd-upload-structure");
      } else {
        Swal.fire({
          icon: "error",
          title: "Gagal mengunggah Struktur BKD!",
          timer: 2000,
          showConfirmButton: false,
          position: "center",
        });
  ...
  const handleUpdateRegulations = async (
    ...
    try {
      const response = await updateBKDStructure(formData, id);
      ...
      if (response.status === 200) {
        ...
        Swal.fire({
          icon: "success",
          title: "Berhasil Mengupdate Struktur BKD!",
          timer: 2000,
          showConfirmButton: false,
          position: "center",
        });
        ...
        Swal.fire({
          icon: "error",
          title: "Gagal Mengupdate Struktur BKD!",
          timer: 2000,
          showConfirmButton: false,
          position: "center",
        });
  ...
  const handleDeleteRegulations = async (id: number) => {
    setIsDeleteLoading(true);
    try {
      const result = await Swal.fire({
        title: "Apakah Anda Yakin Menghapus Struktur BKD?",
        text: "Struktur BKD yang telah dihapus tidak dapat dipulihkan!",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteBKDStructure(id);

        if (response.status === 200) {
          await Swal.fire({
            icon: "success",
            title: `Struktur BKD berhasil dihapus!`,
            timer: 2000,
            position: "center",
          });
      ...
      <div className="bg-line-10 shadow-md rounded-lg w-full flex flex-col p-5 gap-y-5">
                  ...
                  <AlertDialogHeader className="flex flex-col max-h-[500px]">
                    <AlertDialogTitle className="text-center text-[16px]">
                      <AlertDialogDescription className="text-center">
                        Master Data Upload Struktur BKD
                      </AlertDialogDescription>
                    </AlertDialogTitle>
                    ...
                    <form
                      ...
                      <div className="w-full focus-within:text-primary-70 flex flex-col gap-y-3">
                        <Label className="text-neutral-700 font-normal mb-2 text-[16px]">
                          Title
                        </Label>
                        <Input
                          type="text"
                          ...
                          placeholder="Masukkan Judul Struktur BKD"
                        />
                      </div>
        ...
        <div className="w-full">
          {!isMobile ? (
            <>
              {bkds && bkds.length > 0 && (
                <SuperUploadBKDMasterDataTablePages
                  bkds={bkds}
                  data={data}
